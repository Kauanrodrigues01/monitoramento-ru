...
class TestCreateQueueReport_HappyPath:
    async def test_report_created_successfully_returns_queue_report(
        ...
        with (
            ...
            patch(_PATCH_RESTAURANT_SNAPSHOT_UPDATE_TASK),
        ):
            result = await service.create_queue_report(
                restaurant.public_id, valid_payload, _IP, _DEVICE_ID
            )
...
# ── TestCreateQueueReport_CeleryTask ─────────────────────────────────────────


class TestCreateQueueReport_CeleryTask:
    async def test_celery_task_dispatched_after_successful_creation(
        self,
        service,
        mock_repo,
        mock_restaurant_repo,
        ...
        with (
            ...
            patch(_PATCH_HAVERSINE, return_value=50.0),
            patch(_PATCH_CONFIDENCE, return_value=Decimal("1.00")),
            patch(_PATCH_RESTAURANT_SNAPSHOT_UPDATE_TASK) as mock_delay,
        ):
            await service.create_queue_report(
                restaurant.public_id,
                valid_payload,
    ...
    async def test_celery_task_dispatched_with_correct_ru_id(
        self,
        service,
        mock_repo,
        mock_restaurant_repo,
        ...
        mock_delay.assert_called_once_with(restaurant.id)

    async def test_celery_task_not_dispatched_when_restaurant_not_found(
        self,
        service,
        mock_restaurant_repo,
        valid_payload,
        ...
        with (
            patch(_PATCH_RESTAURANT_SNAPSHOT_UPDATE_TASK) as mock_delay,
            pytest.raises(RestaurantNotFoundError),
        ):
            await service.create_queue_report(
                uuid4(),
                valid_payload,
    ...
    async def test_celery_task_not_dispatched_when_geofence_violation(
        self,
        service,
        mock_repo,
        mock_restaurant_repo,
        ...
        with (
            ...
            patch(_PATCH_SETTINGS) as mock_settings,
            patch(_PATCH_RESTAURANT_SNAPSHOT_UPDATE_TASK) as mock_delay,
            pytest.raises(QueueReportLocationOutOfGeofenceError),
        ):
            mock_settings.DEBUG = False
            await service.create_queue_report(
                restaurant.public_id,
    ...
    async def test_celery_task_not_dispatched_on_db_exception(
        self,
        service,
        mock_repo,
        mock_restaurant_repo,
